...
class TransactionsPage {
  ...
  removeAccount() {
    if(this.lastOptions) {
      const accConfirm = confirm('Вы действительно хотите удалить счет?');
      if(accConfirm) {
        const accId = {id: this.lastOptions.account_id}
  ...
  removeTransaction(id) {
    const transConfirm = confirm('Вы действительно хотеите удалить эту транзакцию?');
    if(transConfirm) {
      const transId = {id};
      Transaction.remove(transId, (err, response) => {
        if(response && response.success) {
          App.update();
        }
      })
  ...
  formatDate(date){
    ...
    return `${dateFormat.getDate()} ${month[dateFormat.getMonth()]} ${dateFormat.getFullYear()} г. в ${dateFormat.getHours()}:${dateFormat.getMinutes()}`;
  }
